...
import tracemalloc
from netsquid.util.simtools import possible_time_manipulation_accuracy_issue

__all__ = [
    "logger",
    "get_loggers",
    "warn_time_accuracy_issues",
    "warn_on_time_accuracy",
    "warn_deprecated",
    "check_memory_usage",
]
...
def warn_time_accuracy_issues(message, key='time_accuracy_warn', stacklevel=1):
    """Warn that the simulation time manipulation may incur numeric inaccuracies.
    ...
    if key is None or _deprecation_warnings.get(key) is None:
        warn_message = \
            f'Possible numerical accuracy issues when manipulating time: {message}'
        warnings.warn(warn_message, RuntimeWarning, stacklevel=(stacklevel + 1))
        if key is not None:
            _deprecation_warnings[key] = True


def warn_on_time_accuracy(time1, time2):
    if possible_time_manipulation_accuracy_issue(time1, time2):
        warn_time_accuracy_issues("Delay calculated with {} and {}".format(time1, time2))
