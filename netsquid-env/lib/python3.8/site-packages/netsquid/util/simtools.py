...
__all__ = [
    ...
    "set_random_state",
    "test",
    "possible_time_manipulation_accuracy_issue",
]
...
def set_random_state(seed=None, rng=None):
    ...
    """
    # Ńote that the variable _random_state must be used as a global variable
...
def possible_time_manipulation_accuracy_issue(time1, time2):
    """Checks if numerical operations with the given times can yield accuracy issues

    When manipulating largely different simulation times (e.g. adding
    ...
    in python.
    Take for example the two following sums:
        Sum1 = 1e-3 + 1e18
        Sum2 = 5e-4 + 1e18

    They are obviously different numbers, but in python they yield the same
    numbers.  That can end in, for example, two events that should be scheduled
    at different times, but are triggered at the same time.

    Therefore, some caution should be exercised when running simulations that
    goes for large simulation times.  In fact, when manipulating two time
    intervals that are larger than 1e12 (empirically determined).

    """
    _CRITICAL_TIME_DIFFERENCE = 1e12
    if abs(time1-time2) > _CRITICAL_TIME_DIFFERENCE:
        return True
    return False
